...
newlines = open(PATH_TO_ACL_BIB_FILE, encoding='utf-8').read().split("inproceedings")
...
ORIGINAL_SEARCH_TERM_1 = ["Electronic Health Records", "Patient Discharge",
"Patient Discharge Summaries","health record" , "medical record" , "patient discharge" , 
"discharge record" , "discharge summar" , 
"clinical narrative" , "clinical domain" , "medical domain" , "clinical note" , "clinical record" , 
"clinical text" , "medical text"]

ORIGINAL_SEARCH_TERM_2 = ["Natural Language Processing", "natural language","language processing",
"translation", "NLP","inference", "generation" , "relation extraction", "relation-extraction",
"entity recognition", "entity-recognition", "concept normalization","concept-normalisation",
"relation classification", "sequence label","summarization", "summarisation","parts-of-speech",
"information-retrieval", "information retrieval", "informationgrouping", "sentiment", "semantic",
"temporal-expression","co-reference","word-sense-disambiguation",]

ORIGINAL_SEARCH_TERM_3 = ["similarity", "coherence", "feature"]

ORIGINAL_SEARCH_TERM_4 = ["Datasets as Topic", "shared task", "shared-task", "annotat",
"question-entailment", "question-answering", "challenge", "community", "bakeoff", "bake-off", 
"benchmark", "evaluation","assessment", "track", "public data", "public dataset", "public datasets", 
"publicly available dataset", "publicly available datasets", "public corpus"]
...
OUTPUT_TO_CSV_FULLBIB_SEARCH= "./data/acl_bib_search_fullbib_stem"+str(STEM)+".csv"
OUTPUT_TO_CSV_TITLE_SEARCH = "./data/acl_bib_search_title_stem"+str(STEM)+".csv"
OUTPUT_TO_CSV_ABSTRACT_SEARCH = "./data/acl_bib_search_abstract_stem"+str(STEM)+".csv"

with open(OUTPUT_TO_CSV_FULLBIB_SEARCH,"w", encoding='utf-8') as cout:
    wr = csv.writer(cout)
    wr.writerow(["Title", "Abstract", "Bib Item", "Matched Search Term 1",
    "Matched Search Term 2", "Matched Search Term 4"])
...
if WRITE_RESULTS_TO_CSV:
    with open(OUTPUT_TO_CSV_TITLE_SEARCH,"w", encoding='utf-8') as cout:
        wr = csv.writer(cout)
        wr.writerow(["Title", "Matched Search Term 1", "Matched Search Term 2",
    ...
    with open(OUTPUT_TO_CSV_ABSTRACT_SEARCH,"w", encoding='utf-8') as cout:
        wr = csv.writer(cout)
        wr.writerow(["Tintle", "Abstract", "Matched Search Term 1", "Matched Search Term 2",
         "Matched Search Term 4"])
...
if WRITE_RESULTS_TO_CSV:
    import io
    with io.open("./data/acl_bib_search_term1_cnt_stem"+str(STEM)+".csv","w",
    encoding='utf-8') as cout:
        wr = csv.writer(cout)
        wr.writerow(["Term", "Cnt"])
        for k,v in term1_cnt.items():
            wr.writerow([k, v])

        with open("./data/acl_bib_search_term2_cnt_stem"+str(STEM)+".csv","w",
        encoding='utf-8') as cout:
            wr = csv.writer(cout)
            wr.writerow(["Term", "Cnt"])
        ...
        with open("./data/acl_bib_search_term4_cnt_stem"+str(STEM)+".csv","w",
        encoding='utf-8') as cout:
            wr = csv.writer(cout)
            wr.writerow(["Term", "Cnt"])
